...
export default {
  async fetch(request, env, ctx) {
    ...
    const isSub = ['all', 'vless', 'vmess', 'trojan', 'hysteria', 'hysteria2', 'clash', 'sub', 'subscribe', 'singbox', 'tuic'].some(t => pathPart.includes(t));
    ...
    ['vless', 'vmess', 'hysteria2', 'hysteria', 'trojan', 'ss', 'clash', 'singbox', 'tuic'].forEach(t => {
...
function parseFlatNode(ob) {
    ...
    try {
        ...
        // --- TUIC ---
        if (type === 'tuic') {
             const password = getProp(ob, ['password', 'uuid']);
             const params = new URLSearchParams();
             if (sni) params.set('sni', sni);
             const congestion = getProp(ob, ['congestion', 'congestion_control']);
             if (congestion) params.set('congestion_control', congestion);
             const udpRelay = getProp(ob, ['udp_relay_mode']);
             if (udpRelay) params.set('udp_relay_mode', udpRelay);
             if (insecure === '1') params.set('allow_insecure', '1');
             
             return { l: `tuic://${encodeURIComponent(password||'')}@${server}:${port}?${params}#${encodeURIComponent(tag)}`, p: 'tuic', n: tag };
        }
